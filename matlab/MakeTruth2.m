function [board, Lfull] = MakeTruth2(folder, Lfull, board)
% Usage:  board = MakeTruth2(folder)
% if Lfull already found:
%		 board = MakeTruth2(folder, Lfull)
% to double check an existing labeling:
%		 board = MakeTruth2(folder, [], board)
%		 board = MakeTruth2(folder, Lfull, board)
% to return the board location:
%		 [board, Lfull] = MakeTruth2(...)
%
...
% written by steve scher in 2008, sscher@ucsc.edu
	...
	if nargin < 3 || isempty(board)
		...
		for f = 2:FRAMES
			...
			board(:,:,f) = ClickToUpdateBoard(board(:,:,f-1), im, fig);
		end
	end
