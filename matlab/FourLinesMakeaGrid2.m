...
%		score(spacing,g,1) = nnz(E & bw);
		score(spacing,g,2) = nnz(E & bw)/sqrt(nnz(bw));																	% Best
%		score(spacing,g,3) = nnz(E&bw) - nnz(E & ~bw) - nnz(~E & bw);
%		score(spacing,g,6) = nnz(E&bw & bwBOXfull) / (nnz(E & ~bw & bwBOXfull) + nnz(~E & bw & bwBOXfull))  * 100;		% Second Beset
		...
		elseif 0
			...
			BestScore = 1; SpacingGuess = 1; T = struct([]);
			return;
		end
